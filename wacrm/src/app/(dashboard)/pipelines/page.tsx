...
export default function PipelinesPage() {
  ...
  useEffect(() => {
    if (authLoading || !user || !accountId) return;
    let cancelled = false;
    (async () => {
      setLoading(true);
  ...
  }, [authLoading, user, accountId, loadPipelines, seedDefaultPipeline]);
    ...
    <div className="space-y-6">
      ...
      <Dialog open={newPipelineOpen} onOpenChange={setNewPipelineOpen}>
        <DialogContent className="sm:max-w-sm bg-slate-900 border-slate-700">
          <DialogHeader>
            <DialogTitle className="text-white">Novo Pipeline</DialogTitle>
          </DialogHeader>
          <div className="py-2">
            <Label className="text-slate-300">Nome do Pipeline</Label>
            <Input
