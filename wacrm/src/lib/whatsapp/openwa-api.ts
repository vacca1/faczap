...
// Session lifecycle (create / start / status / QR) lives in the server-only
// client `openwa-server.ts`, which the connect flow uses with credentials
// from the environment. This module keeps only the message-sending helpers
// used by the send/broadcast/react routes.
